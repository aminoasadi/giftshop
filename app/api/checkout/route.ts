...
export async function POST(request: NextRequest) {
  ...
  if (!session?.user?.id) return NextResponse.redirect(new URL("/login", request.url), { status: 303 });
  ...
  try {
    ...
    return NextResponse.redirect(new URL(`/dashboard?order=${order.id}`, request.url), { status: 303 });
  } catch (error) {
    ...
    return NextResponse.redirect(new URL(`/dashboard?error=${encodeURIComponent(message)}`, request.url), { status: 303 });
  }
}
